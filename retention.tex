...
that cutoff. Repeatedly queried items have their scores damped. We call the result the
guarded cutoff rule. It is the operating policy the rest of the paper
...
recommends. We fix $(L,K)=(60,6)$, calibrate the Gaussian
information-equivalent channel to the five-category ordered-probit
Fisher information, and compare five policies at matched budgets
\(N_{\mathrm{tot}}\in\{10L,30L,100L\}\).
...
the current shipment cutoff and gradually loses interest in items it has
already queried many times. Two simpler baselines ignore part of the
decision. Even allocation spreads answers uniformly across the catalog,
and uncertainty-only targeting asks about the item whose answer would
reduce total posterior variance most, wherever that item sits. Two
richer alternatives estimate what our rule approximates. The combined
cutoff \(\times\) uncertainty rule keeps the cutoff focus but replaces
the count damping with the estimated posterior-variance reduction, and
the estimated marginal-value rule implements the priority formula of
Proposition~\ref{prop:local-marginal} directly.

\paragraph{Implementation.}
Let \(\hat c=\hat w_{(K)}\) be the current estimated cutoff and let the kernel score of item \(i\) be
\[
k_i \;=\; 0.10+0.90\exp\!\big(-\big((\hat w_i-\hat c)/h\big)^2\big),
\qquad
h=\max\{\mathrm{med}(g_K),\,0.10\,\mathrm{sd}(\hat{\w}),\,0.08\},
\]
where \(g_K\) holds the two order-statistic gaps adjacent to the cutoff, \(\hat w_{(K-1)}-\hat w_{(K)}\) and \(\hat w_{(K)}-\hat w_{(K+1)}\), and \(\mathrm{sd}(\hat{\w})\) is the cross-sectional standard deviation of the current means. The guarded cutoff rule queries \(\arg\max_i k_i/(1+0.10\,N_i)\), where
\(N_i\) is the number of answers item \(i\) has received. We call the
division by \(1+0.10\,N_i\) the count damping. Every answer an item
receives shrinks its score a little, so an item that has been queried
ten times keeps only half its kernel score and repeatedly queried items
gradually step aside. The combined rule multiplies \(k_i\) by \(0.15+0.85\,u_i/\max_j u_j\) and divides by \(1+0.05\,N_i\), where \(u_i=\sum_j A_{ij}(\N)^2/\big(A_{ii}(\N)+1/\kappa_i\big)\) is the total posterior-variance reduction across the catalog from one more answer on item \(i\). The estimated marginal-value score is \(\kappa_i\sum_j k^0_j\,A_{ij}(\N)^2\), where \(k^0_j\) is the kernel score with the floor removed; under an independent prior this reduces to the product form of the proposition. All constants were chosen once during development and held fixed across every experiment, both priors, and all budgets. The count damping in the guarded rule is an estimate-free stand-in for the remaining-learnability factor in Proposition~\ref{prop:local-marginal}; the combined and marginal-value rules estimate that factor from the posterior instead.

The experiment uses a common warm start of five responses per item and
\(100\) shared synthetic worlds. Table~\ref{tab:alloc-efficacy}
reports mean next-box value gaps with standard errors, and
...
paired improvements relative to even allocation, and within-family
...
\midrule
AR(1) $\rho=0.6$            & $10L$   & $0.297\,(0.033)$ & $0.251\,(0.033)$ & $\mathbf{0.078}\,(0.013)$ & $0.111\,(0.016)$ & $0.083\,(0.015)$ \\
AR(1) $\rho=0.6$            & $30L$   & $0.138\,(0.017)$ & $0.127\,(0.019)$ & $\mathbf{0.022}\,(0.006)$ & $0.062\,(0.011)$ & $0.061\,(0.012)$ \\
AR(1) $\rho=0.6$            & $100L$  & $0.045\,(0.007)$ & $0.048\,(0.008)$ & $\mathbf{0.008}\,(0.002)$ & $0.017\,(0.004)$ & $0.042\,(0.010)$ \\
\midrule
Independent ($\bSigma{=}I$) & $10L$   & $0.400\,(0.037)$ & $0.363\,(0.037)$ & $\mathbf{0.107}\,(0.017)$ & $0.119\,(0.017)$ & $0.113\,(0.018)$ \\
Independent ($\bSigma{=}I$) & $30L$   & $0.209\,(0.025)$ & $0.151\,(0.018)$ & $\mathbf{0.035}\,(0.009)$ & $0.050\,(0.009)$ & $0.080\,(0.015)$ \\
Independent ($\bSigma{=}I$) & $100L$  & $0.050\,(0.010)$ & $0.055\,(0.010)$ & $\mathbf{0.004}\,(0.001)$ & $0.022\,(0.005)$ & $0.061\,(0.014)$ \\
\bottomrule
\end{tabular}
...
The guarded cutoff rule is the best mean policy in all six prior-budget
settings and wins in \(65\) to \(90\) percent of individual worlds
(Appendix Table~\ref{tab:alloc-win-rates}). Cutoff relevance carries
most of the allocation value. Uncertainty-only targeting is weak in
both prior structures because it keeps uncertainty while ignoring
whether that uncertainty can change the next box. Once the platform has
a thin layer of baseline information, much of the value comes from
resolving which products sit near the shipment cutoff.

An ablation isolates what the count damping contributes
