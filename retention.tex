...
\]
where \(g_K\) holds the two order-statistic gaps adjacent to the cutoff, \(\hat w_{(K-1)}-\hat w_{(K)}\) and \(\hat w_{(K)}-\hat w_{(K+1)}\), and \(\mathrm{sd}(\hat{\w})\) is the cross-sectional standard deviation of the current means. The guarded cutoff rule queries \(\arg\max_i k_i/(1+0.10\,N_i)\), where \(N_i\) is the number of answers item \(i\) has received; we call the division the count damping. Each answer an item receives shrinks its score, and ten answers cut it
in half. This stops the rule from spending the whole budget on the same
few borderline items. It also mirrors the theory.
Proposition~\ref{prop:local-marginal} prices an answer as cutoff
relevance times what is left to learn, and an item that has been asked
often has little left to teach, so the answer count stands in for the
second factor. The combined and marginal-value rules estimate that
factor from the posterior; their exact scores are in
Appendix~\ref{app:refinement-tables}. All constants were chosen once during development and held fixed across every experiment, both priors, and all budgets.
