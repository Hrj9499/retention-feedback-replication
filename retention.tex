...
\]
where \(g_K\) holds the two order-statistic gaps adjacent to the cutoff, \(\hat w_{(K-1)}-\hat w_{(K)}\) and \(\hat w_{(K)}-\hat w_{(K+1)}\), and \(\mathrm{sd}(\hat{\w})\) is the cross-sectional standard deviation of the current means. The guarded cutoff rule queries \(\arg\max_i k_i/(1+0.10\,N_i)\), where \(N_i\) is the number of answers item \(i\) has received. We call the division by \(1+0.10\,N_i\) the count damping. Every answer an item receives shrinks its score a little, so an item that has been queried ten times keeps only half its kernel score and repeatedly queried items gradually step aside. Uncertainty-only targeting queries \(\arg\max_i u_i\), where \(u_i=\sum_j A_{ij}(\N)^2/\big(A_{ii}(\N)+1/\kappa_i\big)\) is the total posterior-variance reduction across the catalog from one more answer on item \(i\). The combined rule multiplies \(k_i\) by \(0.15+0.85\,u_i/\max_j u_j\) and divides by \(1+0.05\,N_i\). The estimated marginal-value score is \(\kappa_i\sum_j k^0_j\,A_{ij}(\N)^2\), where \(k^0_j\) is the kernel score with the floor removed; under an independent prior this reduces to the product form of the proposition. All constants were chosen once during development and held fixed across every experiment, both priors, and all budgets. Proposition~\ref{prop:local-marginal} prices an answer as cutoff
