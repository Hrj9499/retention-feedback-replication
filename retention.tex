...
about. Even allocation spreads answers uniformly and ignores the
decision. Uncertainty-only targeting asks about the item whose answer
would reduce total posterior variance most, wherever that item sits.
The guarded cutoff rule asks about items near the current shipment
cutoff and gradually loses interest in items it has already queried
many times. The combined cutoff \(\times\) uncertainty rule keeps
the same cutoff focus but replaces the count damping with the estimated
posterior-variance reduction. The estimated marginal-value rule
implements the priority formula of
...
scales; lower is better). The guarded cutoff rule attains the lowest
gap at every budget under both priors. Standard errors are in
Table~\ref{tab:alloc-efficacy}.}
