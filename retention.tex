...
uncertainty alone can lose even to spreading questions evenly. In
chronological MovieLens ratings, boundary-focused rules beat uniform
sampling and recover between a sixth and two fifths of the lost shipment
value.
\end{abstract}
...
top alternatives. Optimal computing budget allocation decides how many
simulation replications each alternative receives, including subset-selection variants that target the best \(m\) alternatives at once \citep{ChenLinYucesanChick2000,ChickInoue2001,ChenHeFuLee2008}.
...
w_{(K)}\) and score each product by a smooth kernel of its distance to
that cutoff. Repeatedly queried items have their scores damped. This produces the
cutoff \(\times\) uncertainty rule without requiring a separate
calibration of the theoretical density \(b_i(\N)\).
...
This is the main allocation experiment. After the platform has a
small amount of baseline feedback, is it better to spend new responses
near the shipment cutoff than to spread them evenly or chase uncertainty
alone? We fix $(L,K)=(60,6)$ and calibrate the
Gaussian information-equivalent channel to the five-category ordered-probit
Fisher information. At matched budgets \(N_{\mathrm{tot}}\in\{10L,30L,100L\}\), we compare even allocation, uncertainty-only targeting, a guarded cutoff rule, the combined cutoff \(\times\) uncertainty rule, and an estimated marginal-value rule motivated by Proposition~\ref{prop:local-marginal}. The scoring rules are fully specified. Let \(\hat c=\hat w_{(K)}\) be the current estimated cutoff and let the kernel score of item \(i\) be
\[
k_i \;=\; 0.10+0.90\exp\!\big(-\big((\hat w_i-\hat c)/h\big)^2\big),
\qquad
h=\max\{\mathrm{med}(g_K),\,0.10\,\mathrm{sd}(\hat{\w}),\,0.08\},
\]
where \(g_K\) holds the two order-statistic gaps adjacent to the cutoff, \(\hat w_{(K-1)}-\hat w_{(K)}\) and \(\hat w_{(K)}-\hat w_{(K+1)}\), and \(\mathrm{sd}(\hat{\w})\) is the cross-sectional standard deviation of the current means. The guarded cutoff rule queries \(\arg\max_i k_i/(1+0.10\,N_i)\). The combined rule multiplies \(k_i\) by \(0.15+0.85\,u_i/\max_j u_j\) and divides by \(1+0.05\,N_i\), where \(u_i=\sum_j A_{ij}(\N)^2/\big(A_{ii}(\N)+1/\kappa_i\big)\) is the total posterior-variance reduction across the catalog from one more answer on item \(i\). All constants were chosen once during development and held fixed across every experiment, both priors, and all budgets. The count damping in the guarded rule is an estimate-free stand-in for the remaining-learnability factor in Proposition~\ref{prop:local-marginal}; the combined and marginal-value rules estimate that factor from the posterior instead.

The estimated marginal-value column implements the priority formula from Proposition~\ref{prop:local-marginal}. Its score for item \(i\) is \(\kappa_i\sum_j k^0_j\,A_{ij}(\N)^2\), where \(k^0_j\) is the kernel score with the floor removed; under an independent prior this reduces to the product form of the proposition. Its finite-budget performance shows how the marginal-
value priority behaves when repeated as a practical heuristic.

The experiment uses a common warm start of five responses per item and
\(100\) synthetic worlds. Table~\ref{tab:alloc-efficacy} reports mean
next-box value gaps with standard errors. The replication package
reports the corresponding world-level raw outcomes, policy win rates,
paired improvements relative to even allocation, and within-family calibration references.
...
\toprule
Prior & $N_{\mathrm{tot}}$ & Even & Uncertainty-only & \shortstack{Guarded\\cutoff} & \shortstack{Combined cutoff\\$\times$ uncertainty} & \shortstack{Estimated\\marginal value} \\
\midrule
AR(1) $\rho=0.6$            & $10L$   & $0.297\,(0.033)$ & $0.251\,(0.033)$ & $0.078\,(0.013)$ & $0.111\,(0.016)$ & $0.083\,(0.015)$ \\
AR(1) $\rho=0.6$            & $30L$   & $0.138\,(0.017)$ & $0.127\,(0.019)$ & $0.022\,(0.006)$ & $0.062\,(0.011)$ & $0.061\,(0.012)$ \\
AR(1) $\rho=0.6$            & $100L$  & $0.045\,(0.007)$ & $0.048\,(0.008)$ & $0.008\,(0.002)$ & $0.017\,(0.004)$ & $0.042\,(0.010)$ \\
\midrule
Independent ($\bSigma{=}I$) & $10L$   & $0.400\,(0.037)$ & $0.363\,(0.037)$ & $0.107\,(0.017)$ & $0.119\,(0.017)$ & $0.113\,(0.018)$ \\
Independent ($\bSigma{=}I$) & $30L$   & $0.209\,(0.025)$ & $0.151\,(0.018)$ & $0.035\,(0.009)$ & $0.050\,(0.009)$ & $0.080\,(0.015)$ \\
Independent ($\bSigma{=}I$) & $100L$  & $0.050\,(0.010)$ & $0.055\,(0.010)$ & $0.004\,(0.001)$ & $0.022\,(0.005)$ & $0.061\,(0.014)$ \\
\bottomrule
\end{tabular}
\caption{Mean next-box value gaps under alternative ways to spend the same response budget over \(100\) shared synthetic worlds. Lower values are better.}
\label{tab:alloc-efficacy}
\end{table}

The main result is that cutoff relevance carries most of the allocation
value in these experiments. In both prior structures, uncertainty-only targeting is weak
relative to the cutoff-focused alternatives because it keeps uncertainty
while ignoring whether that uncertainty can change the next box. The
guarded cutoff rule is the best mean policy in all six rows and has
world-level win rates between \(0.65\) and \(0.90\)
(Appendix Table~\ref{tab:alloc-win-rates}). Once the platform has a thin layer of baseline information,
much of the value comes from resolving which products sit near the
shipment cutoff.

An ablation isolates what the response-count damping contributes
(replication package). Removing it stalls the rule completely. It keeps querying the same near-cutoff
items, and its
mean gap stays flat at about \(0.30\) to \(0.43\) as the budget grows
from \(10L\) to \(100L\), while the damped rule falls to \(0.008\).
The guarded cutoff rule is therefore itself a two-factor rule in
the sense of Proposition~\ref{prop:local-marginal}. It is cutoff
relevance times a deliberately robust form of diminishing returns. The comparison across columns then tests competing implementations of
the second factor, and the count-based proxy is more robust at scale
than estimating the same factor from the posterior.

An exact myopic benchmark provides a useful comparison
(Appendix~\ref{app:greedy-optimal}). Under the independent prior, the
expected gain from one full answer has a closed form, a knowledge-
gradient-style computation, so the myopic one-answer rule can be run
exactly, with no kernel and no estimation. The exact rule stalls. Its gap is flat at \(0.097\) across all three
budgets, while on the same matched worlds the guarded rule falls from
\(0.073\) to \(0.006\), a seventeenfold difference at the largest
budget. The failure is myopia. An item whose mean is confidently
wrong offers a negligible one-answer gain, so the rule never queries it,
and the mistake becomes permanent. Practical rules must therefore add
exploration to the product form. The guarded rule does this with a
score floor and count damping, which together sweep every contested
item eventually. In the independent-prior benchmark at experimental scale, the guarded
rule also beats the best static allocation found by direct optimization
at every budget (Appendix~\ref{app:greedy-optimal}).

The estimated marginal-value benchmark connects the experiments to
Proposition~\ref{prop:local-marginal}. At the tight \(10L\) budget, it
is close to the guarded cutoff rule and materially better than even or
uncertainty-only allocation. At larger budgets, repeatedly applying the
marginal-value score is less robust than the simpler guarded cutoff rule
and can lag broad allocation. This clarifies the practical role of the proposition.
Cutoff relevance sets the initial priority; posterior uncertainty and
related-product learning help distinguish among near-cutoff products
when those differences are consequential.

The closest off-the-shelf alternative from ranking and selection also
fits this protocol. A sequential OCBA-m rule for optimal-subset
selection, which recomputes its target shares from the current
posterior means, loses to the guarded rule at every budget under both
priors; at \(30L\) under the independent prior the gaps are \(0.112\)
against \(0.027\) (Appendix Table~\ref{tab:ocba}). OCBA-m targets asymptotic correct-selection guarantees, a different
objective from finite-budget next-box value.
...
the most weight. This subsection summarizes when the simple rule
should be refined. Appendix~\ref{app:refinement-tables} reports the
full tables.
...
quarters of them in every scenario. Second, adding the uncertainty term helps in a narrow regime. It helps at the tightest
budgets, and especially when near-cutoff items differ in how much is already known about them.
...
repetition, with standard errors over \(30\) repetitions.
...
\midrule
400  & $0.647$ & $0.105\,(0.025)$ & $70.0\%$ & $0.108\,(0.021)$ & $73.3\%$ \\
1000 & $0.613$ & $0.149\,(0.017)$ & $93.3\%$ & $0.145\,(0.018)$ & $90.0\%$ \\
2000 & $0.614$ & $0.198\,(0.021)$ & $96.7\%$ & $0.236\,(0.023)$ & $96.7\%$ \\
\bottomrule
\end{tabular}
\caption{MovieLens check. Holdout top-\(K\) gaps; improvement and win-rate columns are paired against uniform within the same repetition (30 repetitions).}
...
The uncertainty-only benchmark is weaker. At \(M=1000\) and \(M=2000\),
its mean gaps are \(0.632\) and \(0.683\), compared with uniform gaps of
\(0.613\) and \(0.614\). Its paired improvements over uniform are
...
cutoff-aware allocation removes between a sixth and two fifths of
that loss, with the share growing as the budget rises.
...
MovieLens experiments, and the calibration steps. The public repository
link will be added here before posting. The MovieLens~100K data are
publicly available from GroupLens. The industrial retention log behind
...
\begin{tabular}{lcccccc}
\toprule
Prior & $N_{\mathrm{tot}}$ & Even & \shortstack{Uncertainty-\\only} & \shortstack{Guarded\\cutoff} & \shortstack{Combined cutoff\\$\times$ uncertainty} & \shortstack{Estimated\\marginal value} \\
\midrule
AR(1) $\rho=0.6$            & $10L$  & $0.33$ & $0.37$ & $0.66$ & $0.58$ & $0.64$ \\
AR(1) $\rho=0.6$            & $30L$  & $0.48$ & $0.51$ & $0.83$ & $0.65$ & $0.70$ \\
AR(1) $\rho=0.6$            & $100L$ & $0.62$ & $0.65$ & $0.86$ & $0.76$ & $0.78$ \\
Independent ($\bSigma{=}I$) & $10L$  & $0.29$ & $0.29$ & $0.65$ & $0.56$ & $0.65$ \\
Independent ($\bSigma{=}I$) & $30L$  & $0.37$ & $0.38$ & $0.77$ & $0.67$ & $0.65$ \\
Independent ($\bSigma{=}I$) & $100L$ & $0.69$ & $0.60$ & $0.90$ & $0.78$ & $0.74$ \\
\bottomrule
\end{tabular}
...
but drops both the uncertainty term and the response-count damping; it
...
quoted in Section~\ref{subsec:design-allocation}. It is a sequential
OCBA-m rule for optimal-subset selection, run under the main
benchmark's setting and budgets, with target shares recomputed each
...
deviation. Both rules are run within this benchmark on freshly drawn
worlds, so the guarded gaps differ slightly from
Table~\ref{tab:alloc-efficacy}. The guarded cutoff rule has lower mean
gaps in every row.
...
Table~\ref{tab:exact-rule} reports the result. The adaptive guarded
rule beats the static optimum at every budget, so adaptivity has real
value. The exact myopic one-answer rule stalls at \(0.097\). Myopic one-
answer gains vanish for items whose means are confidently wrong, so they are
never queried. This is the diagnosis behind the main-text discussion. Robust
...
\begin{tabular}{lccc}
\toprule
Budget & Guarded cutoff & \shortstack{Exact myopic\\one-answer} & \shortstack{Best fixed allocation\\(SAA, static)} \\
\midrule
$10L$  & $0.073\,(0.014)$ & $0.097\,(0.014)$ & $0.110\,(0.026)$ \\
$30L$  & $0.022\,(0.005)$ & $0.097\,(0.014)$ & $0.037\,(0.015)$ \\
$100L$ & $0.006\,(0.002)$ & $0.097\,(0.014)$ & $0.009\,(0.004)$ \\
\bottomrule
\end{tabular}
\caption{Exact myopic one-answer rule and static reference at experimental scale (\(L=60\), \(K=6\), independent prior; 100 worlds for the policies, the first 30 for the SAA reference). Mean gaps with standard errors.}
...
shortly before a cancellation labeled as churn. The estimation sample covers about \(52{,}000\) orders placed
between July and November 2025. Estimates come from logistic
regressions with standardized features, so each is an odds ratio per one
standard deviation. Only aggregate, relative quantities are reported, and the underlying log
