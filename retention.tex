...
The functional forms are deliberately generic, a bell curve for
closeness to the cutoff, a floor so that no item is ever ruled out, and
answer counts for diminishing returns; the constants are round
defaults. Let \(\hat c=\hat w_{(K)}\) be the current estimated cutoff and let the proximity score of item \(i\) be
\[
k_i \;=\; 0.10+0.90\exp\!\big(-\big((\hat w_i-\hat c)/h\big)^2\big),
\qquad
h=\max\{\mathrm{med}(g_K),\,0.10\,\mathrm{sd}(\hat{\w}),\,0.08\},
\]
where \(g_K\) holds the two order-statistic gaps adjacent to the cutoff, \(\hat w_{(K-1)}-\hat w_{(K)}\) and \(\hat w_{(K)}-\hat w_{(K+1)}\), and \(\mathrm{sd}(\hat{\w})\) is the cross-sectional standard deviation of the current means; the exponential is a Gaussian kernel with an adaptive bandwidth \(h\). The guarded cutoff rule queries \(\arg\max_i k_i/(1+0.10\,N_i)\), where \(N_i\) is the number of answers item \(i\) has received. We call the division by \(1+0.10\,N_i\) the count damping. Its job is to keep the rule from spending the whole budget on the same few borderline items. Every answer an item receives shrinks its score a little, so an item that has been queried ten times keeps only half its proximity score and steps aside for its neighbors. All constants were chosen once during development and held fixed across every experiment, both priors, and all budgets. Proposition~\ref{prop:local-marginal} prices an answer as cutoff
...
estimate. The combined and marginal-value rules estimate the same quantity from
the posterior instead; their exact scores are in
...
computation, so the myopic one-answer rule can be run exactly, with no proximity
score and no estimation. The exact rule stalls. Its gap is flat at
