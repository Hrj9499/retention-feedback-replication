...
that cutoff. An item's score also fades as it accumulates answers,
...
\]
where \(g_K\) holds the two order-statistic gaps adjacent to the cutoff, \(\hat w_{(K-1)}-\hat w_{(K)}\) and \(\hat w_{(K)}-\hat w_{(K+1)}\), and \(\mathrm{sd}(\hat{\w})\) is the cross-sectional standard deviation of the current means. The guarded cutoff rule queries \(\arg\max_i k_i/(1+0.10\,N_i)\), where \(N_i\) is the number of answers item \(i\) has received. We call the division by \(1+0.10\,N_i\) the count damping. Its job is to
keep the rule from spending the whole budget on the same few borderline
items. Every answer an item receives shrinks its score a little, so an
item that has been queried ten times keeps only half its kernel score
and steps aside for its neighbors. Uncertainty-only targeting queries \(\arg\max_i u_i\), where \(u_i=\sum_j A_{ij}(\N)^2/\big(A_{ii}(\N)+1/\kappa_i\big)\) is the total posterior-variance reduction across the catalog from one more answer on item \(i\). The combined rule multiplies \(k_i\) by \(0.15+0.85\,u_i/\max_j u_j\) and divides by \(1+0.05\,N_i\). The estimated marginal-value score is \(\kappa_i\sum_j k^0_j\,A_{ij}(\N)^2\), where \(k^0_j\) is the kernel score with the floor removed; under an independent prior this reduces to the product form of the proposition. All constants were chosen once during development and held fixed across every experiment, both priors, and all budgets. Proposition~\ref{prop:local-marginal} prices an answer as cutoff
relevance times how much there is still to learn about the item. The
count damping is a shortcut for that second part. The more often an item
has been asked about, the less one more answer can teach, so the raw
answer count already tracks what is left to learn, with nothing to
estimate. The combined and marginal-value rules estimate the same
quantity from the posterior instead.
...
Cutoff relevance carries most of the allocation value. Uncertainty-only
targeting is weak in both prior structures because it keeps uncertainty
while ignoring whether that uncertainty can change the next box. Once
the platform has a thin layer of baseline information, much of the
value comes from resolving which products sit near the shipment cutoff.

Removing the count damping shows why it is there. The undamped rule
stalls completely (replication package). It keeps querying the same
near-cutoff items, and its mean gap stays flat at about \(0.30\) to
\(0.43\) as the budget grows from \(10L\) to \(100L\), while the
guarded rule falls to \(0.008\). The guarded cutoff rule is therefore
itself a two-factor rule in the sense of
Proposition~\ref{prop:local-marginal}. It is cutoff relevance times a
deliberately robust form of diminishing returns. The comparison across
columns then tests competing implementations of the second factor, and
the count damping proves more robust at large budgets than estimating
that factor from the posterior.

An exact myopic benchmark, a rule that maximizes the expected gain from
just the next answer, provides a useful comparison
(Appendix~\ref{app:greedy-optimal}). Under the independent prior, that
expected gain has a closed form, a knowledge-gradient-style
computation, so the myopic one-answer rule can be run exactly, with no
kernel and no estimation. The exact rule stalls. Its gap is flat at
\(0.097\) across all three budgets, while on the same matched worlds
the guarded rule falls from \(0.073\) to \(0.006\), a seventeenfold
difference at the largest budget. The failure is short-sightedness. An
item whose mean is confidently wrong offers a negligible one-answer
gain, so the rule never queries it, and the mistake becomes permanent.
Practical rules must therefore add exploration to the product form. The
guarded rule does this with a score floor and count damping, which
together sweep every contested item eventually. In the
independent-prior benchmark at experimental scale, the guarded rule
also achieves a lower mean gap than the best static allocation found by
direct optimization at every budget, with the clearest margin at the
tight budget (Appendix~\ref{app:greedy-optimal}).
...
marginal-value score is less robust than the simpler guarded cutoff
rule and can even fall behind even allocation. This clarifies the
practical role of the proposition. Cutoff relevance sets the initial
priority; posterior uncertainty and related-product learning help
distinguish among near-cutoff products when those differences are
consequential.
...
selection, which re-splits the remaining budget across items after
each answer using the current posterior means, loses to the guarded
rule at every budget under both priors; at \(30L\) under the
independent prior the gaps are \(0.112\) against \(0.027\)
(Appendix Table~\ref{tab:ocba}). OCBA-m targets asymptotic
correct-selection guarantees, a different objective from finite-budget
next-box value.
